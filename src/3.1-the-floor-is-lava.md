...
Build a simple world: a sprite, some colored platforms, and red "lava." Introduce `if <touching color?> then` — if you touch lava, go back to start. They now have a **maze game**.
...
Each new color is another `if` block. They're building up a pattern: one condition, one response. The conditions are all checked every frame inside that `forever` loop.
