...
**Blocks to point out:** `if/then/else` is under Control — it's a different block than `if/then` (wider, with two mouths). `distance to` is under Sensing. The variable `last distance` remembers where you *were* so it can compare to where you *are now*. They'll need to create this variable (Variables → Make a Variable).
...
This is challenging but their reading level can handle the block nesting, and it's fun to make the reactions dramatic. The size changes give a nice visual cue beyond just the text. In Scratch, the nested if/else blocks visually indent, which actually makes the structure pretty clear.
